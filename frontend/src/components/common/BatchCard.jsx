...
const BatchCard = ({ batch, userRole, onApprove, onReject, onUpdate }) => {
  ...
  // ✅ IMPORTANT: Sync state when batch updates
  useEffect(() => {
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition-all overflow-hidden">
      ...
      <div className="px-6 pt-6 pb-4 border-b border-gray-100 flex justify-between items-start">
        <div>
          ...
          <p className="font-mono font-semibold text-lg text-gray-900 tracking-wider">
            {shortId}
          </p>
        </div>
        ...
        {userRole === 'Manager' ? (
          <div className="space-y-2">
            ...
            <button
              ...
              onClick={() => onUpdate(batch.id, { coffee_type: coffeeType })}
        ...
        {userRole === 'Manager' ? (
          <div className="space-y-2">
            ...
            <button
              ...
              onClick={() => onUpdate(batch.id, { quantity_kg: qty })}
              className="bg-blue-600 text-white px-3 py-1 rounded disabled:bg-gray-400"
            >
              Save Quantity
            </button>
      ...
      {userRole === 'Manager' && batch.status === 'PENDING' && (
        <div className="border-t border-gray-100 p-5 flex gap-3 bg-gray-50">
          <button
            ...
            className="flex-1 bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-2xl"
          >
            Approve
          </button>
          <button
            onClick={() => onReject && onReject(batch.id)}
            className="flex-1 bg-red-600 hover:bg-red-700 text-white font-semibold py-3 rounded-2xl"
          >
            Reject
          </button>
        </div>
      ...
      {userRole === 'Dealer' && batch.status === 'PENDING' && (
        <div className="bg-amber-50 border-t px-6 py-4 text-center text-amber-700 text-sm">
          ⏳ Waiting for Manager Approval
        </div>
      )}

      {userRole === 'Dealer' && batch.status === 'APPROVED' && (
        <div className="bg-green-50 border-t px-6 py-4 text-center text-green-700 text-sm">
          ✅ Approved by Manager
        </div>
      )}

      {userRole === 'Dealer' && batch.status === 'REJECTED' && (
        <div className="bg-red-50 border-t px-6 py-4 text-center text-red-700 text-sm">
          ❌ Rejected by Manager
        </div>
      )}
    </div>
